...
void CrossfaderWidget::placeSlider(int x, int y) {
  addParam(createParam<BefacoSlidePot>(Vec(x, y), module, Crossfader::FADER,
                                       -10.0f, 10.0f, 0.0f));
}

void CrossfaderWidget::placeTrimpots(int x, int y, int horizontalSpacing) {
  addParam(createParam<Trimpot>(Vec(x, y), module, Crossfader::CV_TRIMPOT,
                                -2.0f, 2.0f, 0.0f));
  addParam(createParam<Trimpot>(Vec(x + horizontalSpacing, y), module,
                                Crossfader::EXP_LIN_TRIMPOT, 0.0f, 1.0f, 0.0f));
}
