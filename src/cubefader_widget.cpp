...
CubefaderWidget::CubefaderWidget() {
...

  {
    ...
    panel->setBackground(SVG::load(assetPlugin(plugin, "res/panels/cubefader.svg")));
    addChild(panel);
  }
...
void CubefaderWidget::placeGuiElements() {
  ...
  addParam(createParam<DelvinToggleTwo>(Vec(x+7, y + 60), module, Cubefader::UNI_BI_TOGGLE, 0.0f, 1.0f, 1.0f));
...
void CubefaderWidget::placeAudioInputs(int startingX, int startingY,
  ...
  for (int i = 0; i != Cubefader::INPUT_001; i += 2) {
    ...
    addInput(createInput<SevenHalfKnurled>(Vec(startingX, y), module, inputLeft));
    addInput(
        createInput<SevenHalfKnurled>(Vec(startingX + spacing, y), module, inputRight));
  }
  ...
  for (int i = Cubefader::INPUT_001; i != Cubefader::X_CV; i += 2) {
    ...
    addInput(createInput<SevenHalfKnurled>(Vec(x + spacing, y), module, inputRight));
  }
}

void CubefaderWidget::placeOutput(int x, int y) {
  addOutput(createOutput<SevenHalfKnurled>(Vec(x, y), module, Cubefader::OUTPUT));
}
...
void CubefaderWidget::placeCvInputs(int x, int y, int horizontalSpacing) {
  ...
  addInput(createInput<SevenHalfKnurled>(Vec(x + horizontalSpacing, y), module,
                                 Cubefader::Y_CV));
  addInput(createInput<SevenHalfKnurled>(Vec(x + (horizontalSpacing * 2) + 1, y), module,
                                 Cubefader::Z_CV));
}

void CubefaderWidget::placeTrimpots(int x, int y, int horizontalSpacing) {
  addParam(createParam<CastleTrimpot>(Vec(x, y), module, Cubefader::X_TRIMPOT, -2.0f,
                                2.0f, 1.0f));
  addParam(createParam<CastleTrimpot>(Vec(x+horizontalSpacing, y), module, Cubefader::Y_TRIMPOT, -2.0f,
                                2.0f, 1.0f));
  addParam(createParam<CastleTrimpot>(Vec(x+(horizontalSpacing*2) + 1, y), module, Cubefader::Z_TRIMPOT, -2.0f,
                                2.0f, 1.0f));
}

void CubefaderWidget::placeScrews() {
  addChild(createScrew<MThreeScrew>(Vec(RACK_GRID_WIDTH+1, 2)));
  addChild(createScrew<MThreeScrew>(Vec((box.size.x - 2 * RACK_GRID_WIDTH)+1, 2)));
  addChild(createScrew<MThreeScrew>(
      Vec(RACK_GRID_WIDTH+1, RACK_GRID_HEIGHT - RACK_GRID_WIDTH + 1)));
  addChild(createScrew<MThreeScrew>(Vec((box.size.x - 2 * RACK_GRID_WIDTH)+1,
